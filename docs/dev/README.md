...
For write commands (`SET`, `MSET`, `DEL`, `GETDEL`, `EXPIRE`, `PEXPIRE`, `EXPIREAT`, `PEXPIREAT`, `PERSIST`, `FLUSHDB`), the handler also appends the command to the AOF after a successful execution (see [AOF persistence](#aof-persistence) below).
