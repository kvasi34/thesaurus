...
- `executor/` — bridges `Command` to `Store`; `mod.rs` holds the `Executor` struct and generic command handlers, `string.rs` holds string-type-specific handlers (`GET`, `SET`, `GETDEL`, `MGET`, `DIGEST`), `list.rs` holds list-type-specific handlers, `set.rs` holds set-type-specific handlers (`SADD`, `SMEMBERS`, `SISMEMBER`, `SMISMEMBER`, `SRANDMEMBER`, `SCARD`, `SMOVE`, `SPOP`, `SREM`)
