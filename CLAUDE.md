...
- `command/` — CLI argument parsing (via clap) and RESP2-to-command parsing; `mod.rs` holds the `Command` enum, dispatch, and generic parsers (also used for SADD, SMEMBERS, SCARD, SPOP, SREM), `string.rs` holds `SetCondition`, `SetExpiry`, and SET parsing, `list.rs` holds LSET/LINDEX/LRANGE parsing
- `executor/` — bridges `Command` to `Store`; `mod.rs` holds the `Executor` struct and generic command handlers, `string.rs` holds string-type-specific handlers (`GET`, `SET`, `GETDEL`, `DIGEST`), `list.rs` holds list-type-specific handlers, `set.rs` holds set-type-specific handlers (`SADD`, `SMEMBERS`, `SCARD`, `SPOP`, `SREM`)
...
Supported: `PING`, `GET`, `SET` (NX, XX, IFEQ, IFNE, IFDEQ, IFDNE, GET, EX, PX, EXAT, PXAT, KEEPTTL), `DEL`, `GETDEL`, `DIGEST`, `EXISTS`, `LPUSH`, `RPUSH`, `LPUSHX`, `RPUSHX`, `LPOP`, `RPOP`, `LLEN`, `LINDEX`, `LSET`, `LRANGE`, `SADD`, `SMEMBERS`, `SCARD`, `SPOP`, `SREM`, `EXPIRE`, `PEXPIRE`, `EXPIREAT`, `PEXPIREAT`, `TTL`, `EXPIRETIME`, `PEXPIRETIME`, `PERSIST`, `DBSIZE`, `SELECT`, `FLUSHDB`
